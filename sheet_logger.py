...
def _safe_update_cell(ws, a1: str, value):
    """Aggiorna una singola cella (gspread 6.x richiede [[valore]])."""
    ws.update(a1, [[value]])
...
class SheetLogger:
    ...
    def log_open(self, *, trade_id: str, side: str, entry_price: float, qty: float,
        ...
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        row = {
            "Data/Ora": now, "ID trade": trade_id, "Lato": side, "Stato": "APERTO",
            "Prezzo ingresso": entry_price, "Qty": qty, "SL %": sl_pct,
            "TP1 %": tp1_pct, "TP2 %": tp2_pct, "Prezzo chiusura": "", "Chiusura": "",
            "P&L %": "", "P&L valore": "", "Equity post-trade": "", "Strategia": strategy, "Note": note
        }
    ...
    def log_close(self, *, trade_id: str, close_price: float, close_type: str,
        ...
        """Completa la riga aperta se trovata; altrimenti aggiunge una riga chiusa."""
        id_col = self.trade_ws.col_values(2)  # B = ID trade
        target_row = None
        for i in range(len(id_col)-1, 1, -1):
            if id_col[i-1] == trade_id:
                stato = self.trade_ws.cell(i, 4).value  # D = Stato
        ...
        if isinstance(target_row, int) and target_row > 1:
            ...
            _safe_update_cell(self.trade_ws, f"D{target_row}", "CHIUSO")
            _safe_update_cell(self.trade_ws, f"A{target_row}", now)
            if note:
                _safe_update_cell(self.trade_ws, f"P{target_row}", note)
        else:
            row = {
                "Data/Ora": now, "ID trade": trade_id, "Lato": "", "Stato": "CHIUSO",
                "Prezzo ingresso": "", "Qty": "", "SL %": "", "TP1 %": "", "TP2 %": "",
                "Prezzo chiusura": close_price, "Chiusura": close_type,
                "P&L %": pnl_pct, "P&L valore": pnl_value, "Equity post-trade": equity_after,
